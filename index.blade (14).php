...
<div class="container mx-auto max-w-7xl p-8">
    <div class="flex items-center justify-between mb-8">
        ...
        <div class="flex items-center space-x-4">
            ...
            <a href="{{ route('owner.investors.create') }}" class="bg-gradient-to-r from-purple-600 to-purple-700 hover:from-purple-700 hover:to-purple-800 text-white px-6 py-2 rounded-xl font-semibold transition duration-200 shadow-lg">
                Add Investor
            </a>
        </div>
    </div>
        ...
        <div class="bg-gradient-to-br from-green-500 to-green-700 rounded-2xl p-6 text-white shadow-xl transform hover:scale-105 transition duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">PKR {{ number_format($investors->sum('investment_amount'), 2) }}</p>
                </div>
                <div class="bg-white bg-opacity-20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
        ...
        <div class="bg-gradient-to-br from-blue-500 to-blue-700 rounded-2xl p-6 text-white shadow-xl transform hover:scale-105 transition duration-300">
            <div class="flex items-center justify-between">
                <div>
                    ...
                    <p class="text-3xl font-bold">PKR {{ number_format($investors->avg('investment_amount'), 2) }}</p>
                </div>
                <div class="bg-white bg-opacity-20 rounded-xl p-3">
                    <svg class="w-8 h-8" fill="currentColor" viewBox="0 0 24 24">
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-8 py-4 text-left text-xs font-bold uppercase tracking-wider text-gray-500">Shares</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($investors as $investor)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-8 py-6">
                                <div class="text-lg font-bold text-green-600">PKR {{ number_format($investor->investment_amount ?? 0, 2) }}</div>
                                <div class="text-sm text-gray-500">{{ number_format($investor->ownership_percentage ?? 0, 2) }}% ownership</div>
                            </td>
                            <td class="px-8 py-6">
                                <div class="text-lg font-bold text-blue-600">{{ $investor->total_shares }}</div>
                            ...
                            <td class="px-8 py-6">
                                <div class="flex space-x-2">
                                    <a href="{{ route('owner.investors.show', $investor->id) }}" class="text-blue-600 hover:text-blue-900 font-semibold transition duration-200">View</a>
                                    <a href="{{ route('owner.investors.edit', $investor->id) }}" class="text-indigo-600 hover:text-indigo-900 font-semibold transition duration-200">Edit</a>
