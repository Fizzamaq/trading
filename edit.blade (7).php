...
@section('title', 'Edit Purchase Invoice: ' . $purchase->invoice_number)
@section('content')

<div class="container mx-auto max-w-7xl p-8">
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-4xl font-extrabold text-gray-900">Edit Purchase Invoice: {{ $purchase->invoice_number }}</h1>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden p-8">
        <form action="{{ route('director.purchases.update', $purchase->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                ...
                <div>
                    ...
                    <input type="date" name="invoice_date" id="invoice_date" value="{{ old('invoice_date', $purchase->invoice_date->format('Y-m-d')) }}" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition duration-200">
                </div>
                <div>
                    <label for="credit_days" class="block text-sm font-medium text-gray-700 mb-2">Credit Period (days)</label>
                    <input type="number" name="credit_days" id="credit_days" value="{{ old('credit_days', $purchase->credit_days) }}" min="0" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-orange-500 focus:border-transparent transition duration-200">
                </div>
            </div>
            ...
            <div id="items-container" class="space-y-4">
                @foreach(old('items', $purchase->items) as $key => $item)
                    <div class="item-row grid grid-cols-1 md:grid-cols-6 gap-4 items-center bg-gray-50 p-4 rounded-xl border border-gray-200">
                        <div>
                            ...
                            <input type="text" name="items[{{ $key }}][product_name]" value="{{ is_object($item) ? $item->product_name : $item['product_name'] }}" required
                        ...
                        <div>
                            ...
                            <input type="text" name="items[{{ $key }}][lot_number]" value="{{ is_object($item) ? $item->lot_number : $item['lot_number'] }}" required
                        ...
                        <div>
                            ...
                            <input type="number" name="items[{{ $key }}][quantity]" value="{{ is_object($item) ? $item->quantity : $item['quantity'] }}" step="0.001" min="0.001" required
                        ...
                        <div>
                            ...
                            <input type="number" name="items[{{ $key }}][unit_price]" value="{{ is_object($item) ? $item->unit_price : $item['unit_price'] }}" step="0.01" min="0.01" required
                        ...
                        <div class="text-right">
                            ...
                            <span class="text-lg font-bold text-gray-800 line-total">
                                PKR {{ number_format((is_object($item) ? ($item->quantity * $item->unit_price) : ($item['quantity'] * $item['unit_price'])), 2) }}
                            </span>
                        </div>
            ...
            <div class="flex justify-between items-center mt-6">
                ...
                <div class="text-right text-2xl font-black text-gray-800">
                    Total Amount: <span id="total-amount">PKR {{ number_format($purchase->total_amount, 2) }}</span>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        let itemIndex = {{ count(old('items', $purchase->items)) }};
