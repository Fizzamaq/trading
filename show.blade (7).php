...
<div class="container mx-auto max-w-7xl p-8">
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-4xl font-extrabold text-gray-900">Purchase Invoice: {{ $purchase->invoice_number }}</h1>
        <div class="flex space-x-4">
            <a href="{{ route('director.purchases.edit', $purchase->id) }}" class="bg-indigo-600 hover:bg-indigo-700 text-white px-6 py-2 rounded-lg font-semibold transition duration-200">
                Edit Invoice
            </a>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden p-8">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
                ...
                <p class="text-lg font-bold text-gray-900">{{ $purchase->invoice_number }}</p>
            </div>
            <div>
                <p class="text-sm font-semibold uppercase text-gray-500">Supplier</p>
                <p class="text-lg text-gray-900">{{ $purchase->supplier->name ?? 'N/A' }}</p>
            </div>
            <div>
                <p class="text-sm font-semibold uppercase text-gray-500">Invoice Date</p>
                <p class="text-lg text-gray-900">{{ $purchase->invoice_date->format('M d, Y') }}</p>
            </div>
            <div>
                <p class="text-sm font-semibold uppercase text-gray-500">Due Date</p>
                <p class="text-lg text-gray-900">{{ $purchase->due_date->format('M d, Y') }}</p>
            </div>
            <div>
                <p class="text-sm font-semibold uppercase text-gray-500">Total Amount</p>
                <p class="text-lg font-bold text-green-600">PKR {{ number_format($purchase->total_amount, 2) }}</p>
            </div>
            <div>
                ...
                <span class="inline-flex rounded-full px-2 text-xs font-semibold leading-5 
                    ...
                    @elseif($purchase->status === 'partially_paid') bg-yellow-100 text-yellow-800
            ...
            <table class="min-w-full divide-y divide-gray-200">
                <thead class="bg-gray-50">
                    <tr>
                        ...
                        <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-500">Lot Number</th>
                        <th class="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider text-gray-500">Quantity</th>
                ...
                <tbody class="bg-white divide-y divide-gray-100">
                    @forelse ($purchase->items as $item)
                        <tr class="hover:bg-gray-50 transition-colors duration-200">
                            ...
                            <td class="px-6 py-4 text-gray-600">{{ $item->lot_number }}</td>
                            <td class="px-6 py-4 text-gray-600">{{ $item->quantity }}</td>
                            <td class="px-6 py-4 text-gray-600">PKR {{ number_format($item->unit_price, 2) }}</td>
                            <td class="px-6 py-4 text-gray-900 font-bold">PKR {{ number_format($item->line_total, 2) }}</td>
                        </tr>
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-4 text-center text-gray-400 italic">No items found for this invoice.</td>
                        </tr>
                    @endforelse
                </tbody>
            </table>
