...
@section('title', 'Edit Inventory Item: ' . $inventory->lot_number)
@section('content')

<div class="container mx-auto max-w-7xl p-8">
    <div class="flex items-center justify-between mb-8">
        <h1 class="text-4xl font-extrabold text-gray-900">Edit Inventory Item: {{ $inventory->lot_number }}</h1>
    ...
    <div class="bg-white rounded-2xl shadow-xl overflow-hidden p-8">
        <form action="{{ route('director.inventory.update', $inventory->id) }}" method="POST">
            @csrf
            @method('PUT')
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                    ...
                    <input type="text" name="product_name" id="product_name" value="{{ old('product_name', $inventory->product_name) }}" required
                ...
                <div>
                    ...
                    <input type="text" name="lot_number" id="lot_number" value="{{ old('lot_number', $inventory->lot_number) }}" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                </div>
                <div>
                    <label for="original_quantity" class="block text-sm font-medium text-gray-700 mb-2">Original Quantity</label>
                    <input type="number" name="original_quantity" id="original_quantity" value="{{ old('original_quantity', $inventory->original_quantity) }}" step="0.001" min="0.001" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                </div>
                <div>
                    <label for="remaining_quantity" class="block text-sm font-medium text-gray-700 mb-2">Remaining Quantity</label>
                    <input type="number" name="remaining_quantity" id="remaining_quantity" value="{{ old('remaining_quantity', $inventory->remaining_quantity) }}" step="0.001" min="0" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                </div>
                <div>
                    <label for="unit_cost" class="block text-sm font-medium text-gray-700 mb-2">Unit Cost</label>
                    <input type="number" name="unit_cost" id="unit_cost" value="{{ old('unit_cost', $inventory->unit_cost) }}" step="0.01" min="0.01" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                </div>
                <div>
                    <label for="purchase_date" class="block text-sm font-medium text-gray-700 mb-2">Purchase Date</label>
                    <input type="date" name="purchase_date" id="purchase_date" value="{{ old('purchase_date', $inventory->purchase_date->format('Y-m-d')) }}" required
                           class="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-transparent transition duration-200">
                </div>
            </div>
